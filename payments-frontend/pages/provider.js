...
export default function Form() {
  ...
  const registerUser = (event) => {
    ...
    fetch("https://api.pmonitor.golem.network/v1/provider/" + event.target.apikey.value)
      .then((r) => r.json())
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          ...
          <p className="mt-2 text-sm text-gray-700">A list of all Providers.</p>
        </div>
                ...
                <tbody className="bg-white">
                  {data.map((row) => (
                    <tr key={row.invoice_id} className="bg-gray-50">
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{row.wallet_address}</td>
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <a href="#" className="text-indigo-600 hover:text-indigo-900">
                          View<span className="sr-only">, {row.project_id}</span>
                        </a>
                      </td>
                    </tr>
                  ))}
