...
from api.models import Project, Logs
from rest_framework.response import Response
from rest_framework.permissions import IsAuthenticated
from rest_framework.views import APIView
from django.http import HttpResponse, JsonResponse
from django.db.models import Sum
from api.serializers import PaymentSerializer, ActivitySerializer, LogsSerializer
...
class dashboard(APIView):
    ...
    def get(self, request):
        ...
        logs = Logs.objects.filter(project__in=project).order_by('-id')[:5]
        log_serializer = LogsSerializer(logs, many=True)
        return Response(status=200, data={'logs': log_serializer.data})
